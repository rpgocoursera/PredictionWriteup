...
Since Random Forest is the model with the highest accuracy, the model will be used to evaluate the accuracy on the test data.
```{r revalidate}
predictRF <- predict( modelRF, myTest )
confusionMatrix( predictRF, myTest$classe)
```

From the result above, Random Forest Model accurracy is about the same with the training model result, which means there is no overfitting on the model and can be used to run on the Test set to get the 20 predictions. The results are given below.
